...
<div class="flex flex-col w-full lg:w-240 lg:min-w-150 p-5 card-full-height shadow-lg rounded-lg">
	<div class="p-5 lg:overflow-y-auto">
		<div class="flex flex-col gap-y-6 xl:gap-y-10">
			<div class="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
				<div class="flex items-center space-x-4 md:space-x-6">
					<img :src="require(`@/assets/${ icon }`)" :alt="name" class="w-8 lg:w-12 xl:w-16 h-8 lg:h-12 xl:h-16" />
					<h1 class="font-bold text-2xl lg:text-3xl xl:text-4xl" :style="`color: ${ color }EE;`">{{ name }}</h1>
				</div>
				<div class="flex flex-col md:flex-row items-center md:space-x-6 space-y-4 md:space-y-0">
					...
					<a :href="link" target="_blank" class="mx-auto">
						<button class="flex justify-center items-center px-6 py-2 font-bold text-xl xl:text-2xl text-white rounded-xl space-x-4" :style="`background-color: ${ color };`">
							<font-awesome-icon :icon="[ 'fab', 'github' ]" class="text-2xl xl:text-3xl" />
							<span>GitHub</span>
						</button>
					</a>
				</div>
			...
			<div class="flex-grow">
				<p class="font-semibold text-xl xl:text-2xl whitespace-pre-wrap" :style="`color: ${ color }E3;`">{{ description }}</p>
			</div>
			<div class="md:h-64">
				<img
					...
					class="xl:h-56 mx-auto rounded-lg cursor-pointer"
				/>
			</div>
		</div>
	</div>
