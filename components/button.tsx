...
function Button({ iconSlug, label, onClick }: IButtonProps) {
	...
	return (
		<div className="flex">
			...
			<button type="button" onMouseOver={ () => setHovered(true) } onMouseOut={ () => setHovered(false) } onClick={ onClick } className="flex min-w-50 h-16">
				<div className={ `flex items-center gap-x-4 px-6 py-4 ${ isHovered && "bg-light-0" }` }>
